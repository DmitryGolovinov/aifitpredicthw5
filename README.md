...
- Верхний гайд «что показать на защите» с быстрым сценарием демо.
- Вкладка `Диагностика`: риск, объяснение топ-факторов и экспорт отчёта (`JSON`).
- Вкладка `Сценарии`: сравнение текущего профиля и сценария «приблизить к медианам train».
- Вкладка `Лаборатория факторов`: интерактивный what-if анализ чувствительности риска по выбранному признаку.
- Вкладка `Инсайты модели`: глобальная важность признаков и метрики holdout.

## Структура проекта

- `train.ipynb` — обучение модели и сохранение артефактов
- `model.py` — класс `Predictor` (`predict`, `predict_probability`, `predict_batch`)
...
2. Войдите в [Timeweb Cloud](https://timeweb.cloud/my) и откройте раздел **App Platform**.
